...
@router.get(
    ...
    summary="Fetch cards",
    description="""
    **Primary ingestion endpoint for the Cards Bronze pipeline.**
    ...
    - Card lifecycle changes (Active → Expired → Blocked) are trackable
      via `updated_at` — only changed cards are returned on incremental runs.
